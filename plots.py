...
# ─────────────────────────────────────────
# Time-to-threshold figures
# ─────────────────────────────────────────

def fig_time_to_threshold_detail(
    ttt_concern: dict,
    ttt_elevated: dict,
    icb_name: str,
) -> plt.Figure:
    """
    Posterior first-passage-time distribution for one ICB — dashboard version.

    Shows the probability mass function of the first week at which total
    pressure crosses the concern and elevated thresholds respectively.

    Parameters
    ----------
    ttt_concern  : output of time_to_threshold(..., threshold=THRESHOLD_CONCERN)
    ttt_elevated : output of time_to_threshold(..., threshold=THRESHOLD_ELEVATED)
    icb_name     : display name for the title

    Assumptions documented on the figure
    -------------------------------------
    - AR(1) level rolled forward from current posterior; rho and sigma_drift
      drawn per sample — uncertainty in dynamics is fully propagated.
    - Seasonal effects held fixed at posterior estimates (stable pattern assumed).
    - Future seasons assigned by calendar month (START_DATE + week offset).
    - Total pressure = latent level + season effect.  Observation noise excluded
      (forecasting the latent signal, not a noisy observation).
    - Zero-mean AR(1): no structural drift is modelled; forecast is conditional
      on the current regime persisting.
    - Samples that have already exceeded the threshold at t=0 are counted at week 0.
    """
    horizon = ttt_concern["horizon_weeks"]
    bins    = np.arange(-0.5, horizon + 1.5, 1.0)
    weeks   = np.arange(0, horizon + 1)

    fig, axes = plt.subplots(1, 2, figsize=(13, 4), layout="constrained")

    for ax, ttt, label, color, thresh_label in zip(
        axes,
        [ttt_concern, ttt_elevated],
        ["Concern threshold", "Elevated threshold"],
        ["#d97706", "#b91c1c"],
        [f"latent ≥ {ttt_concern['threshold']:.1f}",
         f"latent ≥ {ttt_elevated['threshold']:.1f}"],
    ):
        crossing = ttt["crossing_week"]
        p_cross  = ttt["p_cross"]
        med_wks  = ttt["median_weeks"]
        p10_wks  = ttt["p10_weeks"]
        p90_wks  = ttt["p90_weeks"]
        p_never  = 1.0 - p_cross

        # PMF of crossing weeks among those that cross
        crossings = crossing[~np.isnan(crossing)]
        if len(crossings) > 0:
            counts, _ = np.histogram(crossings, bins=bins)
            pmf = counts / len(crossing)   # normalise over ALL samples
        else:
            pmf = np.zeros(len(weeks))

        ax.bar(weeks, pmf, color=color, alpha=0.7, width=0.8,
               label=f"P(first crossing at week t)  [{thresh_label}]")

        if med_wks is not None:
            ax.axvline(med_wks, color=color, linewidth=2, linestyle="--",
                       label=f"Median crossing: week {med_wks:.1f}")
        if p10_wks is not None and p90_wks is not None:
            ax.axvspan(p10_wks, p90_wks, alpha=0.12, color=color,
                       label=f"80% range: {p10_wks:.0f}–{p90_wks:.0f} wks")

        ax.set_xlabel("Weeks from now")
        ax.set_ylabel("Posterior probability")
        ax.set_title(f"{label}\nP(cross within {horizon} wks) = {p_cross:.0%}   "
                     f"P(never) = {p_never:.0%}")
    ...
    assumptions = (
        "Assumptions: AR(1) dynamics propagated from current posterior (rho and sigma_drift "
        "drawn per sample). Seasonal pattern held fixed at posterior estimates; future seasons "
        "from calendar dates. Total pressure = latent level + season effect; observation noise "
        "excluded (forecasting signal, not a single reading). Zero-mean AR(1) — no structural "
        "drift modelled. Conditional on current regime persisting."
    )
    fig.suptitle(
        f"Time to threshold — {icb_name}",
        fontsize=12, y=1.01,
    )
    fig.text(0.5, -0.06, assumptions, ha="center", fontsize=7.5,
             color="#4f4f4f", wrap=True,
             transform=fig.transFigure)
    return fig


def fig_time_to_threshold_all_icbs(
    idata: az.InferenceData,
    last_week: int,
    threshold: float,
    threshold_label: str,
    ...
    """
    Horizontal bar chart: each ICB's P(crossing threshold within horizon).

    Bars coloured by risk tier.  Median crossing week (conditional on crossing)
    shown as a dot.  Sorted by P(crossing) descending so highest-risk ICBs
    are at the top.

    Parameters
    ----------
    idata           : InferenceData
    last_week       : last observed week number (for future season mapping)
    threshold       : latent-scale threshold
    threshold_label : human-readable label for the threshold
    horizon_weeks   : weeks ahead to simulate
    icb_filter      : if set, show only this one ICB

    Assumptions documented on the figure
    -------------------------------------
    Same as fig_time_to_threshold_detail.  This figure summarises P(crossing)
    across all ICBs using the same simulation.
    """
    from dashboard_shared import time_to_threshold, resolve_icb_index

    icbs       = [str(n) for n in idata.attrs.get("icbs", [])]
    rng        = np.random.default_rng(42)

    rows = []
    for icb_name in icbs:
        if icb_filter is not None and icb_name != icb_filter:
            continue
        try:
            idx = icbs.index(icb_name)
        except ValueError:
            continue
        ttt = time_to_threshold(
            idata, idx, last_week, threshold, horizon_weeks, rng=rng
        )
        rows.append(dict(
            icb=icb_name,
            p_cross=ttt["p_cross"],
            median_weeks=ttt["median_weeks"] if ttt["median_weeks"] is not None else np.nan,
        ))
    ...
    summary = pd.DataFrame(rows).sort_values("p_cross", ascending=True)
    short   = summary["icb"].str.replace("NHS ", "").str.replace(" ICB", "")
    y       = np.arange(len(summary))
    colors  = [
        "#b91c1c" if p >= 0.5 else "#d97706" if p >= 0.20 else "#15803d"
        for p in summary["p_cross"]
    ]
    fig_h   = max(3.0, len(summary) * 0.55 + 1.5)
    fig, ax = plt.subplots(figsize=(11, fig_h), layout="constrained")

    ax.barh(y, summary["p_cross"], color=colors, alpha=0.75, height=0.65)

    # Dot for median crossing week (right axis concept — annotate instead)
    for yi, (_, row) in zip(y, summary.iterrows()):
        if not np.isnan(row["median_weeks"]):
            ax.text(
                min(row["p_cross"] + 0.01, 0.98), yi,
                f"  med {row['median_weeks']:.0f}w",
                va="center", fontsize=7.5, color="#1d1d1d",
            )

    ax.axvline(0.50, color="#b91c1c", linestyle="--", linewidth=1,
               label="High risk (≥50%)")
    ax.axvline(0.20, color="#d97706", linestyle="--", linewidth=1,
               label="Moderate risk (≥20%)")
    ax.set_yticks(y)
    ax.set_yticklabels(short, fontsize=9)
    ax.set_xlabel(f"P(crossing {threshold_label} within {horizon_weeks} weeks)")
    ax.set_title(
        f"Time-to-threshold — {threshold_label} — {horizon_weeks}-week horizon\n"
        "Colour: red ≥50%, amber ≥20%, green <20%.  Annotation: median weeks to crossing."
    )
    ax.set_xlim(0, 1.05)
    ...
    assumptions = (
        f"Assumptions: AR(1) rolled forward {horizon_weeks} weeks from current posterior. "
        "rho and sigma_drift drawn per sample (uncertainty propagated). "
        "Seasonal effects fixed at posterior estimates; future seasons from calendar dates. "
        "Total pressure = latent level + season effect. Observation noise excluded. "
        "Zero-mean AR(1) — no structural drift. Conditional on current regime persisting."
    )
    fig.text(0.5, -0.04, assumptions, ha="center", fontsize=7.5,
             color="#4f4f4f", transform=fig.transFigure)
    return fig
