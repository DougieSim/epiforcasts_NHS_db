...
class CacheManager:
    ...
    def _extract_samples(idata: az.InferenceData, icb_idx: int) -> np.ndarray:
        ...
        Expects posteriors from the AR(1) model (core.model or core.inference),
        which stores the latent level as ``idata.posterior["level"]`` with shape
        (chains, draws, n_weeks, n_icb). Returns the final week's level for the
        given ICB, flattened across chains and draws.
        """
